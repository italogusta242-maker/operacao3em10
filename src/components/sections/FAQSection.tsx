  ...
  return (
    <section className="py-12 md:py-24 bg-background" id="faq">
      <ScrollReveal className="container max-w-3xl px-5">
        <h2 className="font-display font-bold text-xl md:text-3xl lg:text-4xl text-foreground text-center mb-8 md:mb-10">
          Perguntas que você pode estar se fazendo agora:
        </h2>

        <Accordion type="single" collapsible className="space-y-2 md:space-y-3">
          {faqs.map(({ q, a }, i) => (
            <AccordionItem key={i} value={`faq-${i}`} className="border rounded-xl px-4 md:px-5 bg-card shadow-sm">
